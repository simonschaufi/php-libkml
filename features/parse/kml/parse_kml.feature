Feature: Parse KML
  ...
  Scenario: Parsing a KML Document with a feature
    Given a KML document with a Document in "tests/kml/kml-document.kml"
  ...
  Scenario: Parsing a KML Document with a NetworkLinkControl
    Given a KML document with a Document in "tests/kml/kml-document.kml"
    When I parse the KML document
    Then I should get a KmlDocument
    And the KmlDocument should contain a NetworkLinkControl with:
      | property         | value            |
      | minRefreshPeriod | 60               |
