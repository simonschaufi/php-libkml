...
class AbstractViewExtractorTest extends TestCase {
...
TAG;
  ...
  public function testExtractCamera() {
    $kml = simplexml_load_string(self::KML_CAMERA);

    $view = AbstractViewExtractor::extract($kml);

    $this->assertInstanceOf(Camera::class, $view);
  }

  public function testExtractLookAt() {
    $kml = simplexml_load_string(self::KML_LOOK_AT);

    $view = AbstractViewExtractor::extract($kml);

    $this->assertInstanceOf(LookAt::class, $view);
  }
}
